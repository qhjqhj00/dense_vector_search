...
class DenseRetriever(object):
    ...
    """
    def __init__(self, question_encoder: nn.Module, batch_size: int, tensorizer: Tensorizer, index: DenseIndexer, device):
...
def get_retriever(args):
    ...
    retriever = DenseRetriever(encoder, args.batch_size, tensorizer, index, args.device)
...
def get_idx(retriever, questions: Union[list, str], topk: int):
    ...
    # get top k results
    top_ids_and_scores = retriever.get_top_docs(questions_tensor.numpy(), topk)
    merged_ids_scores = []
    for s in top_ids_and_scores:
        tmp = {}
        for i, candidate_id in enumerate(s[0]):
            full_id = candidate_id.split('_')[0]
            if full_id in tmp:
                continue
            else:
                tmp[full_id] = {'score': float(s[1][i])}
        merged_ids_scores.append(tmp)
    return merged_ids_scores
...
if __name__ == '__main__':
    ...
    questions = ['北京对疫情期间中小企业有何优惠政策？','新冠肺炎是什么？']
    res = get_idx(retriever, questions, topk=10)
    print(res)
